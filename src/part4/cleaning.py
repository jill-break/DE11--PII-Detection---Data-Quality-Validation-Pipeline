...
class DataRemediator:
    ...
    def normalize_names(self):
        """Applies Title Case to names, preserving NaN."""
        for col in ['first_name', 'last_name']:
            # mask for non-null values
            mask = self.df[col].notna()
            before = self.df.loc[mask, col].copy()
            self.df.loc[mask, col] = self.df.loc[mask, col].astype(str).str.strip().str.title()
            
            # Count changes (excluding cases where it was already title case)
            after = self.df.loc[mask, col]
            self.log_stats["name_fixes"] += (before != after).sum()
        return self

    def normalize_emails(self):
        """Converts emails to lowercase."""
        if 'email' in self.df.columns:
            mask = self.df['email'].notna()
            before = self.df.loc[mask, 'email'].copy()
            self.df.loc[mask, 'email'] = self.df.loc[mask, 'email'].astype(str).str.lower()
            
            # Count changes
            after = self.df.loc[mask, 'email']
            self.log_stats["email_fixes"] += (before != after).sum()
        return self
    ...
    def normalize_dates(self):
        ...
        for col in date_cols:
            ...
            # Identify rows that need formatting (not already YYYY-MM-DD or not NaN)
            mask = self.df[col].notna() & ~self.df[col].astype(str).str.match(r'^\d{4}-\d{2}-\d{2}$')
            self.log_stats["date_fixes"] += mask.sum()

            
            self.df[col] = pd.to_datetime(self.df[col], errors='coerce', format='mixed').dt.strftime('%Y-%m-%d')
        return self
    ...
    def generate_log(self, output_path, validation_before, validation_after):
        ...
        log = [
            ...
            f"- Email case: Converted to lowercase ({self.log_stats['email_fixes']} occurrences)",
            "",
            "Missing Values:"
        ]
