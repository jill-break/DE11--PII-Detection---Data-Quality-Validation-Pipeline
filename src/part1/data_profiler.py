...
class DataProfiler:
    ...
    def _check_quality_issues(self):
        ...
        if not phone_variants.empty:
            self.report_lines.append(f"{issue_count}. Non-standard Phone Formats, Examples: {phone_variants['phone'].head(2).tolist()}")
            issue_count += 1
        ...
        for col in date_cols:
            if col in self.df.columns:
                matches = self.df[self.df[col].astype(str).str.contains('/', na=False)][col].unique().tolist()
                slash_dates.extend(matches)
        
        if slash_dates:
            self.report_lines.append(f"{issue_count}. Inconsistent Date Formats (Slashes detected): {slash_dates[:3]}")
            issue_count += 1

        # 3. Name Casing Issues
        bad_casing = []
        for col in ['first_name', 'last_name']:
            if col in self.df.columns:
                # Check for lowercase or all-caps
                mask = self.df[col].notna() & (~self.df[col].astype(str).str.istitle())
                if mask.any():
                    example = self.df[mask][col].iloc[0]
                    bad_casing.append(f"{col} ('{example}')")
        
        if bad_casing:
            self.report_lines.append(f"{issue_count}. Inconsistent Name Casing: {', '.join(bad_casing)}")
            issue_count += 1

        # 4. Email Format/Casing
        upper_emails = self.df[self.df['email'].astype(str).str.contains(r'[A-Z]', na=False)]
        if not upper_emails.empty:
            self.report_lines.append(f"{issue_count}. Non-standard Email Casing (Uppercase detected)")
            issue_count += 1

        # 5. Invalid Category Values
        valid_statuses = ['active', 'inactive', 'suspended']
        invalid_status = self.df[~self.df['account_status'].astype(str).str.lower().isin(valid_statuses) & self.df['account_status'].notna()]
        if not invalid_status.empty:
            self.report_lines.append(f"{issue_count}. Invalid Category Values: {invalid_status['account_status'].unique().tolist()}")
            issue_count += 1

        # 6. Data Integrity
        if not self.df['customer_id'].is_unique:
            self.report_lines.append(f"{issue_count}. Duplicate Customer IDs detected.")
            issue_count += 1
